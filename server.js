...
const server = http.createServer((req, res) => {
  ...
  if (req.method === "POST" && u.pathname === "/api/load") {
    ...
    req.on("end", () => {
      ...
      torrent.once("ready", () => {
        ...
        if (!file) { responded = true; res.writeHead(404); res.end(JSON.stringify({ error: "No video file found", files: torrent.files.map(f => f.name) })); return; }
        activeFile = file;
        file.select();
        console.log("[torrent] ready:", file.name);
        responded = true;
  ...
  if (req.method === "GET" && u.pathname === "/api/status") {
    ...
    res.writeHead(200, { "Content-Type": "application/json" });
    res.end(JSON.stringify({ name: activeTorrent.name, file: activeFile?.name, size: activeFile?.length, progress: activeTorrent.progress, downloadSpeed: activeTorrent.downloadSpeed, numPeers: activeTorrent.numPeers, done: activeTorrent.done }));
    return;
  }
